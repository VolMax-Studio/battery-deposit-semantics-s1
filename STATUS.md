---
project: battery-deposit-semantics-s1
phase: candidate_preregistration_pending_freeze
owner: Ivan
analyst: Sol
ops: Ananke
gate: formal Gate (external)
instrument_file: INSTRUMENT.md
sampling_frame_file: SAMPLING_FRAME.md
pre_freeze_exposure_file: PRE_FREEZE_EXPOSURE.md
prereg_frozen: false
candidate_freeze_commit: null
verdict: null
sample_size: 4
calibration_case: "Sandia (observed case / preliminary calibration)"
calibration_positive: "Chung 2021 (DOI: 10.1038/s41597-021-00954-3)"
confirmatory_positive_status: UNSELECTED_PENDING_FREEZE
confirmatory_targets_status: UNSELECTED_PENDING_FREEZE
target_deposit_inspections_pre_freeze: 0
---

## Instance Summary
This repository contains the preregistration specification and measurement instrument for auditing export semantics and reproducibility boundaries across public lithium-ion battery degradation datasets.

### Current State
- **Phase:** Candidate Preregistration (pre-freeze).
- **Execution State:** HALT (`target_deposit_inspections_pre_freeze = 0`).
- **Preregistration Freeze:** Pending formal Gate review and Operator ratification.
- **Strict Boundary:** No Table 2 dataset repository URLs or target documentation have been opened or evaluated.
